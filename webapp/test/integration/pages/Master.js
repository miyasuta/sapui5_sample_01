						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("ProductKey") === oObject.ProductKey;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].ProductKey;
									} else {
										sCurrentId = aItemsNotInTheList[0].ProductKey;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ProductCollection", {
										ProductKey : sCurrentId
									});
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<ProductCollection> (0)"}),
						success: function () {
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all ProductCollection to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<ProductCollection> (" + iExpectedLength + ")"}),
								success : function () {
